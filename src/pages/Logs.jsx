...
export default function Logs() {
  ...
  const fetchEntries = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from("daily_entries")
      .select("created_at, structured, targets, training_type")
    ...
    data.forEach((entry) => {
      ...
      mapped[dayKey] = {
        ...
        training_type: entry.training_type, // gym | cycle | run | null
      };
    });
  ...
  function getTrainingLabel(entry) {
    const hasTraining =
      entry?.structured?.activities && entry.structured.activities.length > 0;

    if (!hasTraining) return "❌";

    return entry?.structured?.activities[0].training_type;
  }
  ...
  function getDayColor(entry) {
    ...
    if (!hasTraining) return "#2a2a33"; // muted rest day
    ...
    const score = (trainingImproved ? 1 : 0) + targetRatio;
      ...
      for (let i = 0; i < 7; i++) {
        ...
        days.push(
          ...
          >
            ...
            >
              <span>{getTrainingLabel(entry)}</span>
          ...
          >
            ...
            >
              ...
              <p style={{ opacity: 0.85 }}>
                {getTrainingLabel(selectedDay.entry)}
              </p>
